...
async def help01(ctx):
    ...
    embed.add_field(name='mdb!soudayo', value='そうだよ(便乗)と発言します',inline=False)
    embed.add_field(name='------------------------------------------------------------', value='Mermoの便利ツール',inline=False)
    embed.add_field(name='mdb!tra [翻訳する文]', value='日本語⇔英語の翻訳をします',inline=False)
    embed.add_field(name='現在の時刻', value='現在の時刻を知らせます(mdb!は不要です)*1', inline=False)
    embed.add_field(name='------------------------------------------------------------', value='Mermoの状態確認',inline=False)
    embed.add_field(name='mdb!ping', value='MermoとDiscordの接続のPing値を確認します',inline=False)
    embed.add_field(name='mdb!test', value='Mermoのテスト送信を実行します',inline=False)
    embed.add_field(name='------------------------------------------------------------', value='Mermoのメンバー管理',inline=False)
    embed.add_field(name='mdb!kick [キックするメンバーの名前またはユーザーIDまたはメンション]', value='Mermoがメンバーをキックします', inline=False)
    embed.add_field(name='mdb!ban [キックするメンバーの名前またはユーザーIDまたはメンション]', value='MermoがメンバーをBANします', inline=False)
    embed.add_field(name='------------------------------------------------------------', value='Mermo PictureBot',inline=False)
    embed.add_field(name='mdb!me', value='Mermoが送信できる画像の説明とコマンドを表示します',inline=False)
    embed.add_field(name='------------------------------------------------------------', value='Mermo Global Chat Service',inline=False)
    embed.add_field(name='mdb!mrmgc', value='Mermoのグローバルチャット(Embed版)の利用方法について表示します',inline=False)
    embed.add_field(name='!mglobal', value='Mermoのグローバルチャット(Webhook版)の登録処理をします(グローバルチャットを行うチャンネルで実行してください)',inline=False)
    embed.add_field(name='------------------------------------------------------------', value='Mermo荒らしコマンド',inline=False)
    embed.add_field(name='mdba!name50', value='実行者の名前を50回発言します', inline=False)
    embed.add_field(name='mdba!name100', value='実行者の名前を100回発言します', inline=False)
    embed.add_field(name='mdba!mentions100', value='実行者へ100回メンションします', inline=False)
    embed.add_field(name='------------------------------------------------------------', value='Mermoの情報',inline=False)
    embed.add_field(name='mdb!botver', value='Mermoのバージョンを確認します', inline=False)
    embed.add_field(name='mdb!pastud', value='過去のアップデートを確認します', inline=False)
    embed.add_field(name='mdb!invite', value='Mermoをサーバーに招待リンクを送信します', inline=False)
    embed.add_field(name='mdb!joinserver', value='Mermoのサポートサーバーへ参加します', inline=False)
    embed.add_field(name='mdb!boti', value='Mermoの動作環境などを表示します', inline=False)
    embed.add_field(name='------------------------------------------------------------', value='Mermo Helpページ',inline=False)
    embed.add_field(name='*マークの説明', value='*1:常時稼働の機能ではないため、一時的に停止する場合があります\n*2:荒らし禁止のサーバー・チャンネルでは実行しないでください\n*3:実行するには管理者権限が必要です')
